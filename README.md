...
| per-actor tick (subset) | 5-scenario × per-frame field trace (timers, anim gate, attack-windup, player regen, status/corpse timers) vs **real `h.a(j,long,boolean)` bytecode** | byte-identical |
