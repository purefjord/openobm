...
That is everything needed to **play**. The fixture-gated test suites additionally
want `tests/fixtures/`, which is generated by the validation harness and is not
published; without it those suites fail for want of their inputs, while the
...
cargo test --workspace                                          # pure-logic tests
