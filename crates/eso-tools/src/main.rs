...
use eso_tools::{
    ...
    dump_scr_trace, dump_targeting_sweep, dump_tick_sweep, dump_xp_sweep, save_roundtrip,
...
fn main() -> Result<()> {
    ...
    let out = match args[0].as_str() {
        ...
        // dot-sweep / corpse-sweep / ai-sweep are self-contained (synthetic actors + RNG).
        "dot-sweep" => dump_dot_sweep()?,
        "corpse-sweep" => dump_corpse_sweep()?,
        "ai-sweep" => dump_ai_sweep()?,
        ...
        // cast-sweep takes the captured hf_tables.txt (the cast ends in h.f).
        "cast-sweep" => dump_cast_sweep(&args[1])?,
